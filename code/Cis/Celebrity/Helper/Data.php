...
/**
 * Celebrity data helper
 */
namespace Cis\Celebrity\Helper;
...
{
    /**
     * Path to store config where count of celebrity posts per page is stored
     *
     * @var string
     */
    const XML_PATH_ITEMS_PER_PAGE     = 'celebrity/view/items_per_page';
    ...
    public function getParentCategories()
    {
        ...
        $qryStr = 'SELECT celebritycategory_id, title FROM cis_celebritycategory where categoryStatus = 1 and categoryChild = 0';        
    ...
    public function getChildCategories($parentId)
    {
        $objectManager =   \Magento\Framework\App\ObjectManager::getInstance();
        $connection = $objectManager->get('Magento\Framework\App\ResourceConnection')->getConnection('\Magento\Framework\App\ResourceConnection::DEFAULT_CONNECTION'); 
        $qryStr = 'SELECT celebritycategory_id, title FROM cis_celebritycategory where categoryStatus = 1 and categoryChild = 1 and categoryParentId = '.$parentId;        
        $results = $connection->fetchAll($qryStr);
        return $results;
    }
    /**
     * Remove Celebrity item image by image filename
     *
     ...
     * @return bool|string
     */
    public function resize(\Cis\Celebrity\Model\Celebrity $item, $width, $height = null)
    {
        if (!$item->getImage()) {
            return false;
        }
    ...
    /**
     * Return the base media directory for Celebrity Item images
     *
     * @return string
     */
    ...
    /**
     * Return the Base URL for Celebrity Item images
     *
     * @return string
     */
     ...
     * @return int
     */
    public function getCelebrityPerPage()
    {
